OC.L10N.register(
    "twofactor_nextcloud_notification",
    {
    "Please accept the request on one of your logged in devices." : "Zatwierdź żądanie na jednym z zalogowanych urządzeń.",
    "You will be redirected automatically once this login has been accepted." : "Zostaniesz przekierowany automatycznie po zaakceptowaniu tego logowania.",
    "Please wait …" : "Proszę czekać …",
    "Your login attempt was rejected." : "Twoja próba logowania została odrzucona.",
    "Use two-factor authentication via Nextcloud notifications" : "Użyj uwierzytelniania dwuskładnikowego za pomocą powiadomień Nextcloud",
    "TwoFactor Nextcloud notification" : "Uwierzytelnianie dwuskładnikowe za pomocą powiadomień Nextcloud",
    "Approve" : "Zatwierdź",
    "Cancel" : "Anuluj",
    "Login attempt from IP address {ip}" : "Próba logowania z adresu IP {ip}",
    "If you are currently trying log in from another device or browser please approve the request. If you are not trying to log in at the moment, you should use the cancel option to abort the login attempt." : "Jeśli obecnie próbujesz zalogować się z innego urządzenia lub przeglądarki, zatwierdź żądanie. Jeśli w tej chwili nie próbujesz się zalogować, powinieneś użyć opcji anuluj, aby przerwać próbę logowania.",
    "Nextcloud Notification" : "Powiadomienia Nextcloud",
    "Authenticate using a device that is already logged in to your account" : "Uwierzytelniaj za pomocą urządzenia, które jest już zalogowane na Twoim koncie",
    "Two-Factor Authentication via Nextcloud notification" : "Dwuskładnikowe uwierzytelnianie za pomocą powiadomienia Nextcloud",
    "Allows using any of your logged in devices as second factor" : "Pozwala na użycie dowolnego z zalogowanych urządzeń jako drugiego czynnika"
},
"nplurals=4; plural=(n==1 ? 0 : (n%10>=2 && n%10<=4) && (n%100<12 || n%100>14) ? 1 : n!=1 && (n%10>=0 && n%10<=1) || (n%10>=5 && n%10<=9) || (n%100>=12 && n%100<=14) ? 2 : 3);");
